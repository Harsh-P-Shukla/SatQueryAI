from inference_config import BACKEND_DIR, BASE_MODEL_ID, MODEL_REVISION, PUBLIC_MODE, MODEL_MODE, MAX_PIXELS, YOLO_DEVICE, YOLO_PATH, require_weights
...
LOG_FILE = str(BACKEND_DIR / "pipeline_debug.log")
...
# CONFIG
CAPTION_ADAPTER_PATH = str(BACKEND_DIR / "adapters/caption_adapter")
VQA_ADAPTER_PATH     = str(BACKEND_DIR / "adapters/vqa_adapter")
# Optional: if images are local instead of URL
IMAGE_DIR = str(BACKEND_DIR / "uploads")
# To store grounded outputs
SAV_DIR = str(BACKEND_DIR / "results")
...
def load_multi_adapter_model():
    ...
    if not has_cuda and not PUBLIC_MODE:
        raise RuntimeError("The fine-tuned 4-bit configuration needs a CUDA GPU. Use SATQUERY_MODEL_MODE=public for CPU demo mode.")
    if not PUBLIC_MODE:
        for directory in [CAPTION_ADAPTER_PATH, VQA_ADAPTER_PATH]:
            require_weights(os.path.join(directory, "adapter_model.safetensors"))
    ...
    if quant_config is not None:
        model_kwargs.update({
            ...
            "device_map": {"": 0} if PUBLIC_MODE else "auto",
    ...
    if not has_cuda:
        base_model.to("cpu")
    logger.info("Base model loaded%s.", " in 4-bit" if quant_config is not None else " on CPU")

    if PUBLIC_MODE:
        base_model.eval()
        logger.warning("Public baseline: no fine-tuned caption/VQA or SAR/IR adapters; DOTA detection only.")
...
def load_image(image_id, image_url=None):
    ...
    if os.path.basename(image_id) != image_id or "/" in image_id or "\\" in image_id:
        raise HTTPException(status_code=400, detail="image_id must be a filename")
...
def parse_multiple_output_lines(output_text):
    """Parse class + eight coordinates + optional region, including repeated values."""
    results = []
    for line in output_text.strip().splitlines():
        parts = line.strip().strip("`").split()
        for start in range(1, len(parts) - 7):
            try:
                coords = [float(value) for value in parts[start:start + 8]]
            except ValueError:
                continue
            if not all(np.isfinite(coords)) or not all(0 <= value <= 1 for value in coords):
                continue
            results.append({"class": " ".join(parts[:start]), "coords": coords,
                            "region": " ".join(parts[start + 8:]) or None})
            break
    return results
...
def generate_with_adapter(model, processor, adapter_name, image,
    ...
    if PUBLIC_MODE:
        pass  # Explicitly selected base-model profile, with no custom adapters.
    elif img_type in ["SAR", "IR"]:
        auto_adapter = pick_adapter(img_type)  # sar or ir
        if auto_adapter not in model.peft_config:
            raise HTTPException(status_code=422, detail=f"The {auto_adapter} adapter is not installed")
...
def answer_grounding(model, processor, image, instruction: str, merged_polys, merged_cls, merged_source, output_path, img_type):
    ...
    system_prompt = """You are an expert vision-language model.
                        ...
                        2. Return only boxes whose object class matches the query.
                        3. If no positional/attribute cues exist: return ALL matching boxes.
    ...
    # Keep only actual detector boxes; the language model must not invent geometry.
    candidates = parse_multiple_output_lines(yolo_output)
    verified = []
    for candidate in candidates:
        if any(item["class"].lower() == candidate["class"].lower()
               and np.allclose(item["coords"], candidate["coords"], atol=1e-5, rtol=0)
               for item in parsed):
...
yolo_dota = YOLO(require_weights(YOLO_PATH))
yolo_dior = None if PUBLIC_MODE else YOLO(require_weights(BACKEND_DIR / "dior_only_last.pt"))

@app.get("/health")
async def health():
    return {"status": "ok", "mode": MODEL_MODE, "base_model": BASE_MODEL_ID,
            "cuda": torch.cuda.is_available(), "detector": "YOLO11x-OBB",
            "dior": yolo_dior is not None, "custom_adapters": not PUBLIC_MODE}
...
async def upload(data: UploadRequest):
    ...
    # The public profile is an RGB baseline, not a radar/SAR interpretation model.
    img_type = "RGB" if PUBLIC_MODE else detect_image_type(os.path.join(IMAGE_DIR, data.image_id))
    img_bgr = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR)  
    pred_dota = yolo_dota.predict(img_bgr, verbose=False, device=YOLO_DEVICE)
    if yolo_dior is None:
        polys, classes = extract_pred_components(pred_dota)
        return {"img_type": img_type, "merged_polys": [] if polys is None else polys.tolist(),
                "merged_cls": [] if classes is None else classes.tolist(),
                "merged_source": [] if classes is None else ["DOTA"] * len(classes)}
    pred_dior = yolo_dior.predict(img_bgr, verbose=False, device=YOLO_DEVICE)
...
async def upload(data: GroundingRequest):
    if os.path.basename(data.output_path) != data.output_path or "/" in data.output_path or "\\" in data.output_path:
        raise HTTPException(status_code=400, detail="output_path must be a filename")
    image = load_image(data.image_id, image_url=data.image_url)
