...
> **Windows local setup:** follow [SETUP_STATUS.md](SETUP_STATUS.md) for verified versions, setup/start commands and actual API routes. The configured public Qwen + YOLO profile runs without the missing custom LFS weights; see [MODEL_SOURCES.md](MODEL_SOURCES.md) for its limitations.
## SatQuery AI — Satellite Query AI Assistant
...
**SatQuery AI** is an advanced AI-powered platform designed for multimodal inference on earth observation and general imagery. It seamlessly integrates image captioning, visual grounding, translation, and query-based analysis into a high-performance web interface.
...
SatQuery AI bridges the gap between complex computer vision tasks and user interaction. It utilizes a **Node.js** and **Python** backend for heavy inference orchestration and a **React-based** frontend for the user experience.
...
The `scripts/` directory contains the complete pipeline used to train, validate, and benchmark the models underlying SatQuery AI. Follow the steps below to reproduce the results.
...
For automated benchmarking and structured reasoning, SatQuery AI offers a **Unified Multimodal Evaluation API**. This endpoint acts as an orchestrator, chaining multiple inference tasks (Captioning, Grounding, VQA) into a single request-response cycle.
